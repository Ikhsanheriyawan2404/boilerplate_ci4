...
class Report extends BaseController
{
    ...
    public function index()
    {
        return view('reports/index', [
            ...
            'assets' => $this->accounts->where('subgroup_account_id', 1)
                ->select('sum(debit - credit) as total, name, code, debit, credit, saldo')
                ->findAll(),
            'liabilities' => $this->accounts->where('subgroup_account_id', 2)
                ->select('sum(credit - debit) as total, name, code, debit, credit, saldo')
                ->findAll(),
            'equities' => $this->accounts->where('subgroup_account_id', 3)
                ->select('sum(credit - debit) as total, name, code, debit, credit, saldo')
                ->findAll(),
            'revenues' => $this->accounts->where('subgroup_account_id', 4)
                ->select('sum(credit - debit) as total, name, code, debit, credit, saldo')
                ->findAll(),
            'cost_of_sales' => $this->accounts->where('subgroup_account_id', 5)
                ->select('sum(debit - credit) as total, name, code, debit, credit, saldo')
                ->findAll(),
